...
- **An empty market search now explains itself, and `tradeable_only` is a client parameter.** `GET /api/sdk/markets` returns `empty_reason` (`filtered_untradeable` | `filtered_expired` | `no_matches`) with a `hint` and `matched_before_tradeable_filter`, but `get_markets()` returns a plain list at the default `response_mode`, so every one of those fields was discarded before the caller saw them — an agent got a bare `[]` and could not tell "no such markets exist" from "they exist, and the default tradeability filter dropped them". Worse, the server's own hint says to retry with `tradeable_only=false` and the client had no such parameter, so it named an action no caller could take. `get_markets()` / `find_markets()` now return a `MarketList` — a `list` subclass, so `len`, iteration, indexing, slicing, `== []` and `isinstance(x, list)` are all unchanged — carrying `empty_reason`, `hint` and `matched_before_tradeable_filter`, with a `repr` that shows the reason when the list is empty. `get_markets(tradeable_only=False)` is the documented retry; the parameter is only sent on the wire when you opt out, so existing callers' requests are byte-identical. Compact/TOON modes use the server's sentence instead of the hardcoded "No markets matched your filters.", which was the wrong sentence precisely when markets *did* match. Same parameter added to the `simmer_get_markets` MCP tool. Fixes the Grok Bot dogfood report where `find_markets("temperature")` returned 0 while bitcoin still hit. (simmer#2292, SIM-5170)
...
### simmer-mcp
- **3.5.2: the initialize handshake no longer waits on the runtime probe.** The server awaited its python/git probe before `server.connect`, so a slow or wedged interpreter could push the handshake past a host's startup timeout (Codex: 10s, dropped silently). Connect now happens first; the version banner prints immediately; the probe runs after and can no longer exit a serving process. Bound by `tests/startup-handshake.test.ts`, which fails on the old ordering. On a handshake-only launch (`openclaw mcp doctor`, `claude mcp list`) the `runtime:` line may now be absent from the log because the host ends the process before the probe finishes; the tool count banner is still there.

- **New keyless tool `simmer_register_agent` (6th keyless tool).** Wraps `POST /api/sdk/agents/register` — no API key required. An agent can now register itself and receive an `api_key`, `agent_id`, `claim_code`, and a claim URL in a single MCP call, without leaving its runtime. Returns a $SIM starting balance for immediate paper trading; the claim URL unlocks real venues. The startup banner keyless count updates from 5 to 6.
...
- **New `polymarket-dca-eval-trader` skill scaffold (SIM-2623).** A staged-entry (3-tranche) DCA template with configurable per-market / daily exposure caps, a dynamic SL/TP model, and an eval-envelope simulator that reports whether proposed sizing stays inside a prop-firm-shaped constraint set (10% target, 6% static / 3% daily drawdown). Paper-safe by default (`--live` required). Ships in the `simmer-sdk` repo `skills/` directory; **status `scaffold`, not yet published to ClawHub and not in the pip wheel** (skills are distributed via ClawHub, not the SDK package). Carries explicit disclaimers — no claim of passing any prop challenge.
